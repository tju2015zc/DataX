...
/**
 * @author Peter
 * @createTime Created in 2022/1/10 17:56
 * @description:
 */
...
public class KafkaReader extends Reader {
    ...
    public static class Job extends Reader.Job {
        ...
        public void init() {
            ...
            logger.info("Kafka Reader Job config: {}", JSON.toJSONString(this.originalConfig));
            ...
            if (columnCount == null) {
                throw DataXException.asDataXException(KafkaReaderErrorCode.PARTITION_ERROR, "没有设置参数[columnCount].");
            } else if (columnCount < 1) {
                throw DataXException.asDataXException(KafkaReaderErrorCode.KAFKA_READER_ERROR, "[columnCount]不能小于1.");
            }
            if (null == split) {
                throw DataXException.asDataXException(KafkaReaderErrorCode.KAFKA_READER_ERROR, "[split]不能为空.");
            }
            if (filterContaintsStr != null) {
                if (conditionAllOrOne == null || filterContaintsFlag == null) {
                    throw DataXException.asDataXException(KafkaReaderErrorCode.KAFKA_READER_ERROR, "设置了[filterContaintsStr],但是没有设置[conditionAllOrOne]或者[filterContaintsFlag]");
                }
            }
            if (parsingRules == null) {
                throw DataXException.asDataXException(KafkaReaderErrorCode.KAFKA_READER_ERROR, "没有设置[parsingRules]参数");
            } else if (!parsingRules.equals("regex") && parsingRules.equals("json") && parsingRules.equals("split")) {
                throw DataXException.asDataXException(KafkaReaderErrorCode.KAFKA_READER_ERROR, "[parsingRules]参数设置错误，不是regex，json，split其中一个");
            }
            if (writerOrder == null) {
                throw DataXException.asDataXException(KafkaReaderErrorCode.KAFKA_READER_ERROR, "没有设置[writerOrder]参数");
            }
            if (kafkaReaderColumnKey == null) {
                throw DataXException.asDataXException(KafkaReaderErrorCode.KAFKA_READER_ERROR, "没有设置[kafkaReaderColumnKey]参数");
            }
        }
        ...
        @Override
        public void post() {
            logger.info("Kafka Reader Job post");
        }

        @Override
        public void destroy() {
            logger.info("Kafka Reader Job destroy");
        }
    }
    ...
    public static class Task extends Reader.Task {
        ...
        public void init() {
            ...
            keyDeserializer = this.readerSliceConfig.getString(Key.KEYDESERIALIZER);
            valueDeserializer = this.readerSliceConfig.getString(Key.VALUEDESERIALIZER);
            logger.info("Kafka Reader Task config: {}", JSON.toJSONString(this.readerSliceConfig));
        }
        ...
        public void startRead(RecordSender recordSender) {
            ...
            props.put("group.id", StringUtils.isNotBlank(groupId) ? groupId : UUID.randomUUID().toString());
            props.put("key.deserializer", StringUtils.isNotBlank(keyDeserializer) ? keyDeserializer : "org.apache.kafka.common.serialization.StringDeserializer");
            props.put("value.deserializer", StringUtils.isNotBlank(valueDeserializer) ? valueDeserializer :  "org.apache.kafka.common.serialization.StringDeserializer");
        ...
        @Override
        public void post() {
            logger.info("Kafka Reader Task post");
        }

        @Override
        public void destroy() {
            flag = false;
            logger.info("Kafka Reader Task destroy");
        }
